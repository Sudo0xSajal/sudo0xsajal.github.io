/* ═══════════════════════════════════════════════════
   tools.js — Tool data + canvas visualizations
═══════════════════════════════════════════════════ */

const TOOLS = [
  {
    id: 'NMAP', n: 'NMAP', c: 'RECON',
    d: 'Radar-style host discovery & port scanning across local networks.',
    draw(cv, t) {
      ...
      x.fillStyle = 'rgba(0,242,255,0.06)'; x.fill(); x.restore();
      x.beginPath(); x.moveTo(o.x, o.y);
      x.lineTo(o.x + Math.cos(angle) * w * .46, o.y + Math.sin(angle) * h * .46);
      x.strokeStyle = 'rgba(0,242,255,0.7)'; x.lineWidth = 1.5; x.stroke();
      [.25,.38,.46].forEach(r => { x.beginPath(); x.arc(o.x,o.y,Math.min(w,h)*r,0,Math.PI*2); x.strokeStyle='rgba(0,242,255,0.07)'; x.lineWidth=.7; x.stroke(); });
      hosts.forEach((h2, i) => {
      ...
      x.fillStyle='rgba(0,242,255,0.4)'; x.font='7px JetBrains Mono';
      x.fillText(`HOSTS:${hosts.length} SCAN:${Math.round((angle/(Math.PI*2))*100)}%`,4,h-5);
    }
  },
  {
    id: 'BURP', n: 'BURP SUITE', c: 'WEB_PROXY',
    d: 'Intercept, replay & fuzz HTTP/S requests to discover web vulnerabilities.',
    draw(cv, t) {
      ...
      const colors=['rgba(0,242,255,.75)','rgba(57,255,20,.75)','rgba(255,51,85,.8)','rgba(255,204,51,.75)','rgba(192,132,252,.7)','rgba(255,51,85,.9)','rgba(57,255,20,.7)'];
      for(let i=0;i<6;i++){
      ...
      x.fillStyle='rgba(255,204,51,0.45)'; x.font='7px JetBrains Mono';
      x.fillText(`REQ:${cnt} | XSS:${Math.floor(t*.3)%8} | SQLi:${Math.floor(t*.2)%4}`,4,h-5);
    }
  },
  {
    id: 'MSPL', n: 'METASPLOIT', c: 'EXPLOIT',
    d: 'Automated vulnerability exploitation framework with payload delivery.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      const root={x:w*.5,y:h*.12};
      const mods=[{x:.2,y:.42,l:'exploit'},{x:.5,y:.42,l:'auxiliary'},{x:.8,y:.42,l:'post'}];
      ...
      mods.forEach((m,i)=>{
        ...
        x.beginPath(); x.moveTo(root.x,root.y); x.lineTo(mx2,my2); x.strokeStyle='rgba(255,51,85,0.28)'; x.lineWidth=1; x.stroke();
        const px2=root.x+(mx2-root.x)*pulse,py2=root.y+(my2-root.y)*pulse;
        x.beginPath(); x.arc(px2,py2,3,0,Math.PI*2); x.fillStyle='rgba(255,204,51,0.9)'; x.fill();
        pls[i].forEach((pl,j)=>{
          const plx=pl.x*w,ply=pl.y*h;
          x.beginPath(); x.moveTo(mx2,my2); x.lineTo(plx,ply); x.strokeStyle='rgba(192,132,252,0.2)'; x.lineWidth=.8; x.stroke();
          const ppx=mx2+(plx-mx2)*((t*.55+.5)%1),ppy=my2+(ply-my2)*((t*.55+.5)%1);
        ...
        x.beginPath(); x.arc(mx2,my2,5.5,0,Math.PI*2); x.fillStyle='rgba(255,51,85,0.75)'; x.fill();
        x.fillStyle='rgba(255,140,66,0.75)'; x.font='7px JetBrains Mono'; x.fillText(m.l,mx2-15,my2-9);
      });
  ...
  {
    id: 'WIRE', n: 'WIRESHARK', c: 'PACKET_ANALYSIS',
    d: 'Deep packet inspection & live network traffic capture.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      const protos=['TCP','UDP','HTTP','DNS','ARP','ICMP','TLS','SSH'];
      const cols={'TCP':'rgba(0,242,255,.7)','UDP':'rgba(57,255,20,.7)','HTTP':'rgba(255,140,66,.7)','DNS':'rgba(192,132,252,.7)','ARP':'rgba(255,204,51,.7)','ICMP':'rgba(255,51,85,.7)','TLS':'rgba(0,242,255,.5)','SSH':'rgba(57,255,20,.5)'};
      const rh=14,rows=8;
      for(let i=0;i<rows;i++){
        ...
        x.fillStyle=hl?'rgba(255,204,51,.85)':(cols[proto]||'rgba(0,242,255,.5)');
        x.font='7.5px JetBrains Mono'; x.fillText(proto,3,i*rh+10);
        x.fillStyle='rgba(0,242,255,0.28)';
        x.fillText(`${192+row%2}.168.${i}.${1+row*3%254}→10.0.${row%5}.${2+i*7%254}`,36,i*rh+10);
        x.fillStyle='rgba(0,242,255,.18)'; x.fillText(`${64+row*37}B`,w-32,i*rh+10);
      }
      x.fillStyle='rgba(255,204,51,0.38)'; x.font='7px JetBrains Mono';
      x.fillText('PROTO  SRC→DST                       LEN',3,h-4);
    }
  },
  {
    id: 'SQLM', n: 'SQLMAP', c: 'SQL_INJECTION',
    d: 'Automated SQL injection detection, exploitation & database extraction.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      ...
      x.fillStyle='rgba(0,0,0,0.35)'; x.fillRect(0,3,w,15); x.strokeStyle='rgba(0,242,255,0.15)'; x.lineWidth=.5; x.strokeRect(0,3,w,15);
      x.fillStyle='rgba(0,242,255,0.45)'; x.font='7.5px JetBrains Mono'; x.fillText('http://target.com/item?id=',3,13);
      x.fillStyle='rgba(255,51,85,0.9)';
      x.fillText(pls[row].slice(0,Math.floor((t*4)%pls[row].length+1)),3+168,13);
      ...
      x.fillStyle='rgba(255,204,51,0.42)'; x.font='7px JetBrains Mono';
      x.fillText(`DB:mysql | TABLES:${3+row} | COLS:${5+row*2}`,3,h-4);
    }
  },
  {
    id: 'HYDR', n: 'HYDRA', c: 'BRUTE_FORCE',
    d: 'Fast online password cracker for SSH, FTP, HTTP and 50+ protocols.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      const users=['admin','root','user','guest','test'];
      const pass=['admin','123456','password','root','test123'];
      ...
      for(let i=0;i<6;i++){
        ...
        x.fillStyle=hit?'rgba(57,255,20,.9)':cur?'rgba(255,204,51,.85)':'rgba(0,242,255,.28)';
        x.font=`${cur?8:7}px JetBrains Mono`;
        x.fillText(`[${cur?'>>':(hit?'✓ ':' ')} ] ${u}:${p}${hit?' ACCESS GRANTED':''}`,4,29+i*17);
      }
      x.fillStyle='rgba(255,204,51,0.4)'; x.font='7px JetBrains Mono';
      x.fillText(`TARGET:SSH:22 | ${Math.round(4+Math.sin(t)*2)} att/s`,3,h-4);
    }
  },
  {
    id: 'FFUF', n: 'FFUF', c: 'WEB_FUZZER',
    d: 'Fast web fuzzer for directory brute-forcing & parameter discovery.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      ...
      const scol={200:'rgba(57,255,20,.8)',404:'rgba(0,242,255,.28)',403:'rgba(255,51,85,.7)',301:'rgba(255,204,51,.7)',302:'rgba(255,140,66,.7)'};
      const cur=Math.floor(t*3)%paths.length;
      x.fillStyle='rgba(0,242,255,0.38)'; x.font='7.5px JetBrains Mono'; x.fillText('http://target.com/FUZZ',3,11);
      x.strokeStyle='rgba(0,242,255,0.1)'; x.lineWidth=.5; x.beginPath(); x.moveTo(0,14); x.lineTo(w,14); x.stroke();
      for(let i=0;i<paths.length;i++){
        ...
        x.fillStyle=done?(scol[sts[i]]||'rgba(0,242,255,.5)'):active?'rgba(255,204,51,.9)':'rgba(0,242,255,.2)';
  ...
  {
    id: 'HASH', n: 'HASHCAT', c: 'PASSWORD_CRACK',
    d: 'GPU-accelerated hash cracking via dictionary, rules & brute-force.',
    draw(cv, t) {
      const x=cv.getContext('2d'),w=cv.width,h=cv.height; x.clearRect(0,0,w,h);
      ...
      hashes.forEach((h2,i)=>{
        const done=prog>(i+1)*.22; const cur=!done&&prog>i*.22;
        x.fillStyle=done?'rgba(57,255,20,.55)':cur?'rgba(255,204,51,.75)':'rgba(0,242,255,.22)';
        x.font='7.5px JetBrains Mono';
        x.fillText(`${h2.slice(0,16)}...→${done?cracked[i]:cur?cracked[i].slice(0,Math.floor(prog*16)):'???'}`,3,13+i*13);
      });
      x.fillStyle='rgba(255,204,51,0.45)'; x.font='7px JetBrains Mono';
      x.fillText(`${Math.round(1850+Math.sin(t*2)*200)} MH/s | rockyou.txt`,3,h-4);
    }
  },
];
...
function renderTools() {
  ...
  TOOLS.forEach(tool => {
    ...
    c.innerHTML = `<div class="tc-h"><span class="tc-n">${tool.n}</span><span class="tc-c">${tool.c}</span></div><canvas class="tc-cv" id="tc-${tool.id}"></canvas><div class="tc-f"><div class="tc-d translatable">${tool.d}</div><div class="tc-s">● ACTIVE</div></div>`;
    g.appendChild(c);
    setTimeout(() => {
      const cv = document.getElementById('tc-' + tool.id); if (!cv) return;
      let t = 0;
      function loop() { cv.width = cv.offsetWidth || 260; cv.height = 130; t += .016; tool.draw(cv, t); requestAnimationFrame(loop); }
      loop();
    }, 80);
  });
}
